app.controller('CalculatorController', function($scope, $timeout) {
  ...
  $scope.getResult = function(form) {
    ...
    var fields = [
      ...
      { key: 'transactional_advice', value: $scope.getTransactionalAdvice() }
    ];
    ...
    $scope.strategy_variation.forEach(function(e) {
      if(e.area_look_at == 1) {
        fields.push({ key: 'strategy_variation[' + ctr + '][name]', value: e.label });
        fields.push({ key: 'strategy_variation[' + ctr + '][value]', value: $scope.getAddFeeNoGST(e) });
        ctr++;
      }
    });
    ctr = 0;
    $scope.strategy_management.forEach(function(e) {
      if(e.area_look_at == 1) {
        fields.push({ key: 'strategy_management[' + ctr + '][name]', value: e.label });
        fields.push({ key: 'strategy_management[' + ctr + '][value]', value: $scope.getAddFeeNoGST(e) });
        ctr++;
      }
    });
  ...
  $scope.deleteNote = function() {
    $scope.note.note = "";
    $scope.note.title = "Add Note";
    $scope.note.action = "add";
